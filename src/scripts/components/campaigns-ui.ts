    ...
    store.campaigns.forEach(campaign => {
        ...
        if (campaign.levels.length === 0) {
            ...
            sortedLevels.forEach((level, idx) => {
                ...
                // Para Legacy: mostrar solo botón de eliminar
                // Para otras campañas: mostrar botones de mover y eliminar
                if (campaign.isDefault) {
                    // Legacy: permitir eliminar niveles
                    const removeBtn = document.createElement('button');
                    removeBtn.className = 'nes-btn is-error text-xs';
                    removeBtn.textContent = t('campaigns.remove');
                    removeBtn.disabled = sortedLevels.length <= 1; // No permitir eliminar si solo queda un nivel
                    removeBtn.addEventListener('click', async () => {
                        ...
                        exitTitleEl.textContent = t('campaigns.remove');
                        exitTextEl.textContent = `¿Seguro que deseas eliminar el nivel ${level.levelIndex + 1}? Esta acción no se puede deshacer.`;
                        exitModalEl.classList.remove('hidden');
                        
                        const confirmHandler = async () => {
                            const success = removeLevelFromCampaign(store, campaign.id, level.levelIndex);
                            if (success) {
                                // Guardar cambios de Legacy en localStorage y archivo
                                const { buildChunkedFile20x18 } = await import('../utils/levels');
                                const levelsAsStrings = store.levelDataStore.map((lvl: string[][]) => 
                                    lvl.map((row: string[]) => row.join(''))
                                );
                                ...
                                try {
                                    ...
                                    console.log('Nivel eliminado de Legacy y guardado en localStorage');
                                } catch (localError) {
                                    console.error('Error guardando en localStorage:', localError);
                                }
                                
                                // Intentar guardar en el archivo del proyecto
                                try {
                                    ...
                                    if (response.ok) {
                                        console.log('Archivo levels.json actualizado después de eliminar nivel');
                                    }
                                } catch (apiError) {
                                    console.warn('Endpoint /api/save-levels no disponible:', apiError);
                                }
                                
                                // Actualizar UI
                                updateCampaignsModal(store);
                                
                                // Si estamos en el editor, actualizar el selector de niveles
                                if (store.appState === 'editing') {
                                    const levelSelectorEl = store.dom.ui.levelSelectorEl;
                                    if (levelSelectorEl) {
                                        // Sincronizar el selector con la campaña actualizada
                                        syncLevelSelectorForCampaign(store);
                                        
                                        // Si el nivel actual era el eliminado o uno después, cambiar al anterior
                                        const currentLevel = parseInt(levelSelectorEl.value || '0', 10);
                                        if (currentLevel >= level.levelIndex || currentLevel >= store.levelDataStore.length) {
                                            const newIndex = Math.max(0, Math.min(level.levelIndex - 1, store.levelDataStore.length - 1));
                                            if (newIndex >= 0 && store.levelDataStore[newIndex]) {
                                                levelSelectorEl.value = newIndex.toString();
                                                levelSelectorEl.dispatchEvent(new Event('change'));
                                            }
                                        }
                                    }
                                }
